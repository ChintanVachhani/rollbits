...
public class PostMessageToGroupService {

    Pipe.Route route;
    Channel clientChannel;

    public PostMessageToGroupService(Pipe.Route route, Channel clientChannel) {
        this.route = route;
    ...
    public void init() {
        for (Node node : RoutingMap.getInstance().getExternalServers().values()) {
            PostMessageToGroupClient postMessageToGroupClient = new PostMessageToGroupClient(node.getNodeAddress(), node.getNodePort(), route, this);
        }
    }
